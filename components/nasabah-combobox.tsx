            ...
            <div className="flex items-center gap-2 text-left">
              ...
              <div>
                <div className="font-medium">
                  {selectedNasabah.person?.nama}
                </div>
                <div className="text-sm text-gray-500">
                  Saldo: Rp {selectedNasabah.saldo.toLocaleString()}
                </div>
              </div>
            </div>
          ) : (
          ...
          <CommandList>
            ...
            <CommandGroup>
              {nasabahList.map((nasabah) => (
                <CommandItem
                  key={nasabah.id}
                  value={`${nasabah.person?.nama} ${nasabah.person?.email} ${nasabah.person?.telepon}`}
                  onSelect={() => {
                ...
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value === nasabah.id ? "opacity-100" : "opacity-0",
                    )}
                  />
                  <User className="h-4 w-4 text-gray-500" />
                  <div className="flex-1">
                    <div className="font-medium">{nasabah.person?.nama}</div>
                    <div className="text-sm text-gray-500">
                      {nasabah.person?.email} • Saldo: Rp{" "}
                      {nasabah.saldo.toLocaleString()}
                    </div>
                  </div>
                </CommandItem>
              ))}
