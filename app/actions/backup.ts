...
export async function getBackupData(bankSampahId: string) {
  try {
    ...
    const [bankSampah, nasabahList, inventarisList, transaksiList] = await Promise.all([
      // Bank Sampah Info
    ...
    const totalNilaiInventaris = inventarisList.reduce((sum, i) => sum + i.stokKg * i.hargaPerKg, 0)

    const transaksiSummary = {
      totalPemasukan: transaksiList.filter((t) => t.jenis === "PEMASUKAN").reduce((sum, t) => sum + t.totalNilai, 0),
    ...
    return {
      ...
      summary: {
        ...
        ...transaksiSummary,
        keuntungan: transaksiSummary.totalPenjualan - transaksiSummary.totalPemasukan,
      },
